...
static int init_listener_tls_ex(nng_listener l, int auth_mode){
	nng_tls_config *cfg;
	int             rv;
...
static int init_listener_tls(nng_listener l){
...
int main(){
  ...
  printf("nng_rep_open::%d\n",nng_rep0_open(&s1));
  printf("nng_listener_create::%d\n",nng_listener_create(&l, s1, addr));
  printf("init_listener_tls::%d\n",init_listener_tls(l));  
  printf("nng_listener_setopt_int_KEEPALIVE::%d\n",nng_listener_setopt_int(l, NNG_OPT_TCP_KEEPALIVE, 1));
  printf("nng_listener_start::%d\n",nng_listener_start(l, 0));
  ...
  for (;;) {
    ...
    count++;
    printf("counter 1 %d\n",count);
    printf("nng_recvmsg :: %s\n", nng_strerror(nng_recvmsg(s1, &msg, 0)));
    printf("counter 2 %d\n",count);
    printf("nng_send :: %s \n",nng_strerror(nng_send(s1, "world", 6, 0)));
    /*
	...
	continue;
      ...
      rv = nng_send(s1, buf, sz, NNG_FLAG_ALLOC);
      if (rv != 0) {
	fatal("nng_send", rv);
      }
      continue;
